...
class LinkPredictor(nn.Module):
    ...
    def predict_all_drugs_for_disease(
        ...
        # Uses actual MDKG relation names from DPKG_types_Cor4.json
        if relation in ['treats', 'treatment_for', 'prevents'] and self.G is not None:
            filter_rels = ['risk_factor_of', 'associated_with', 'characteristic_of']
            drugs_to_penalize = set()
            
            for rel_name in filter_rels:
                etype_check = ('drug', rel_name, 'disease')
                if etype_check in self.G.canonical_etypes:
                    try:
                        src_curr, _ = self.G.in_edges(disease_idx, etype=etype_check)
                        if len(src_curr) > 0:
                            drugs_to_penalize.update(src_curr.tolist())
                    except Exception:
                        pass
                # Also check reverse edges: disease --rev_X--> drug
                rev_etype = ('disease', f'rev_{rel_name}', 'drug')
                if rev_etype in self.G.canonical_etypes:
                    try:
                        _, dst_curr = self.G.out_edges(disease_idx, etype=rev_etype)
                        if len(dst_curr) > 0:
                            drugs_to_penalize.update(dst_curr.tolist())
                    except Exception:
                        pass
            
            if drugs_to_penalize:
                mask_indices = torch.tensor(list(drugs_to_penalize), device=scores.device, dtype=torch.long)
                scores[mask_indices] *= 0.1  # Penalize, don't zero out completely
    ...
    def predict_all_diseases_for_drug(
        ...
        if relation in ['treats', 'treatment_for', 'prevents'] and self.G is not None:
            filter_rels = ['risk_factor_of', 'associated_with', 'characteristic_of']
            diseases_to_penalize = set()
            
            for rel_name in filter_rels:
                etype_check = ('drug', rel_name, 'disease')
                if etype_check in self.G.canonical_etypes:
                    try:
                        _, dst_curr = self.G.out_edges(drug_idx, etype=etype_check)
                        if len(dst_curr) > 0:
                            diseases_to_penalize.update(dst_curr.tolist())
                    except Exception:
                        pass
            
            if diseases_to_penalize:
                mask_indices = torch.tensor(list(diseases_to_penalize), device=scores.device, dtype=torch.long)
                scores[mask_indices] *= 0.1
