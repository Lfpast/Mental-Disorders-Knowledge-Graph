...
class BaseTrainer:
    ...
    def __init__(self, args: argparse.Namespace, config: BertConfig): #DKS
        ...
        if hasattr(args, 'Names') and args.Names:
            self._log_path = os.path.join(self.args.log_path, self.args.Names)
        else:
        ...
        if hasattr(args, 'save_path') and args.save_path:
            if hasattr(args, 'Names') and args.Names:
                self._save_path = os.path.join(self.args.save_path, self.args.Names)
            else:
    ...
    def _save_best(self, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, optimizer: Optimizer,
        ...
        if accuracy > self._best_results[label]:
            self._logger.info(">>> [%s] New Best Model (Iter: %s): %.2f%% accuracy" % (label, iteration, accuracy * 100))
